...
#define v_0 (complex)1                        // velocity of the particles
...
using complex_vector_2d = std::vector<std::vector<std::complex<double>>>;
...
void initiate(double x[], double y[], double nx_old[],
              ...
              complex_vector& Ax, complex_vector& Ay,
              complex_vector& A1, complex_vector& Wk,
              complex_vector& Ax_complex, complex_vector& Ay_complex,
              complex_vector& A1_complex,
              complex_vector& Wk_complex) {
...
void renew(complex_vector_2d& G, complex_vector_2d& omega,
           complex_vector_2d& Flux) {
  int i, j;
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
      G[i][j] = 0;
...
void renew2(complex_vector& WK_m) {
  int i;
  for (i = 0; i < m_max; i++) {
    WK_m[i] = 0;
  }
...
#if 1
  for (i = 0; i < N; i++) {
    x[i] = x[i] + nx_old[i] * tau;
    y[i] = y[i] + ny_old[i] * tau;
  }
#endif
}
...
int main() {
  ...
  double r[N];
  double v[T];
  complex_vector Ax(T), Ay(T), A1(T), Wk(T);
  complex_vector Ax_complex(T), Ay_complex(T), A1_complex(T),
      Wk_complex(T);                   // konjugiert komplex
  complex_vector_2d G(3), omega(3); // ueberpruebfbare Matrizen
  for (int i = 0; i < 3; ++i){
      G[i].resize(3);
      omega[i].resize(3);
  }
  complex_vector_2d Flux(3);           // Flux Matrix
  for (int i = 0; i < 3; ++i){
    Flux.resize(T);
  }
  double nxsum;
  ...
  double norm;
  double v_total;
  complex A1_average, Ax_average, Ay_average;
  complex A1_average_complex, Ax_average_complex, Ay_average_complex;
  double kx = wavevectorx * 2 * M_PI / L, ky = wavevectory * 2 * M_PI / L;
  complex_vector WK_m(m_max);
  ...
  for (l = d_min; l < d_max; l++) {
    ...
    for (time = 0; time < T; time++) {
      ...
      for (j = 0; j < N; j++) {
        A1[time] +=
            (cos(kx * x[j] + ky * y[j]) + 1i * sin(kx * x[j] + ky * y[j]));
        Ax[time] +=
            (cos(kx * x[j] + ky * y[j]) + 1i * sin(kx * x[j] + ky * y[j])) *
            nx_old[j] * v_0;
        Ay[time] +=
    ...
    for (t = waitingtime; t < T - 1; t++) {
      Flux[0][t] = -1 / tau * 1i * (A1_complex[t] - A1_complex[t + 1]);
      Flux[1][t] = -1 / tau * 1i * (Ax_complex[t] - Ax_complex[t + 1]);
      Flux[2][t] = -1 / tau * 1i * (Ay_complex[t] - Ay_complex[t + 1]);
    }
    A1_average /= T;
    Ax_average /= T;
    Ay_average /= T;
    ...
    for (j = 0; j < m_max; j += 1) {
      ...
      WK_m[j] /= p;
      fprintf(fp2, "%d;%f;%f;%f;%f\n", j, WK_m[j].real(), WK_m[j].imag(),
              log(WK_m[j].real()), log(WK_m[j].imag()));
    }
    ...
    fprintf(fp3, "%f;%f;%f;", strength * m, G[0][0].real(), G[0][0].imag());
    fprintf(fp3, "%f;%f;%f;%f;", G[0][1].real(), G[0][1].imag(), G[1][0].real(),
            G[1][0].imag());
    fprintf(fp3, "%f;%f;%f;%f;", G[0][2].real(), G[0][2].imag(), G[2][0].real(),
            G[2][0].imag());
    fprintf(fp3, "%f;%f;%f;%f;", G[2][1].real(), G[2][1].imag(), G[1][2].real(),
            G[1][2].imag());
    fprintf(fp3, "%f;%f;%f;%f\n", G[1][1].real(), G[1][1].imag(),
            G[2][2].real(), G[2][2].imag());

    fprintf(fp4, "%f;%f;%f;", strength * m, omega[0][0].real(), omega[0][0].imag());
    fprintf(fp4, "%f;%f;%f;%f;", omega[0][1].real(), omega[0][1].imag(),omega[1][0].real(), omega[1][0].imag());
    fprintf(fp4, "%f;%f;%f;%f;", omega[0][2].real(), omega[0][2].imag(),omega[2][0].real(), omega[2][0].imag());
    fprintf(fp4, "%f;%f;%f;%f;", omega[2][1].real(), omega[2][1].imag(),omega[1][2].real(), omega[1][2].imag());
    fprintf(fp4, "%f;%f;%f;%f\n",omega[1][1].real(), omega[1][1].imag(),omega[2][2].real(), omega[2][2].imag());
